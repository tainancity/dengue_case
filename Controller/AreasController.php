    ...
    public function export($model = '', $theDate = '') {
        ...
        switch ($model) {
            case 'All':
                ...
                $dailyCase = $this->DailyCase->find('first', array(
                    'conditions' => array(
                ...
                $result[] = array('本土', '境外');
                if (!empty($dailyCase)) {
                    $result[] = array($dailyCase['DailyCase']['count_local'], $dailyCase['DailyCase']['count_imported']);
                } else {
                    $result[] = array('0', '0');
                }
                $result[] = array();
                $result = array_merge($result, $this->Area->csvFever($theDate));
                $result[] = array();
                $result = array_merge($result, $this->Area->csvFever($theDate));
                $result[] = array();
                $result = array_merge($result, $this->Area->csvTrack($theDate));
                $result[] = array();
                $result = array_merge($result, $this->Area->csvCenterSource($theDate));
                $result[] = array();
                $result = array_merge($result, $this->Area->csvAreaSource($theDate));
                $result[] = array();
                $result = array_merge($result, $this->Area->csvVolunteerSource($theDate));
                $result[] = array();
                $result = array_merge($result, $this->Area->csvEducation($theDate));
                $result[] = array();
                $this->loadModel('BureauSource');
                $result = array_merge($result, $this->BureauSource->csvBureauSource($theDate));
                $result[] = array();
                $result = array_merge($result, $this->Area->csvChemical($theDate));
                $result[] = array();
                $result = array_merge($result, $this->Area->csvFeverMonitor($theDate));
                $result[] = array();
                $result = array_merge($result, $this->Area->csvClinicReport($theDate));
                break;
            case 'Expand':
                $result = $this->Area->csvExpand($theDate);
                break;
            case 'Fever':
                $result = $this->Area->csvFever($theDate);
                break;
            case 'Track':
                $result = $this->Area->csvTrack($theDate);
                break;
            case 'CenterSource':
                $result = $this->Area->csvCenterSource($theDate);
                break;
            case 'AreaSource':
                $result = $this->Area->csvAreaSource($theDate);
                break;
            case 'VolunteerSource':
                $result = $this->Area->csvVolunteerSource($theDate);
                break;
            case 'Education':
                $result = $this->Area->csvEducation($theDate);
                break;
            case 'BureauSource':
                $result = $this->Area->csvBureauSource($theDate);
                break;
            case 'Chemical':
                $result = $this->Area->csvChemical($theDate);
                break;
            case 'FeverMonitor':
                $result = $this->Area->csvFeverMonitor($theDate);
                break;
            case 'ClinicReport':
                $result = $this->Area->csvClinicReport($theDate);
                break;
        }
